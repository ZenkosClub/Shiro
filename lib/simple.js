import { proto, generateForwardMessageContent, generateWAMessageFromContent, downloadContentFromMessage, areJidsSameUser, jidDecode } from '@whiskeysockets/baileys'
import fs from 'fs'
...
export function attachConnHelpers(conn) {
  ...
  if (!conn.getName) conn.getName = async function(jid = '', withoutContact = false) {
    ...
    if (jid.endsWith('@g.us')) {
      ...
      v = jid === '0@s.whatsapp.net' ? { vname: 'WhatsApp' } : areJidsSameUser(jid, this.user?.id) ? this.user : (this.chats?.[jid] || {})
